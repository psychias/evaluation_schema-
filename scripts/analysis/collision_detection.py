    ...
    for source_dir in sorted(DATA_DIR.iterdir()):
        ...
        for fpath in source_dir.rglob("*.json"):
            ...
            for result in rec.get("evaluation_results", []):
                ...
                # prefer per-result harness if known, fall back to top-level eval_library
                harness_result = _get_gen_field(gen_cfg, "harness")
                harness_known = harness_result and harness_result not in ("unknown", "")
                harness = harness_result if harness_known else harness_top
