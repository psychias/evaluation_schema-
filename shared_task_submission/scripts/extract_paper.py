...
using pdfplumber, and converts found benchmarks to EEE schema JSON files.
...
# ---------------------------------------------------------------------------
# S: table extractor — pdfplumber table detection only
# ---------------------------------------------------------------------------
...
class TableExtractor:
    ...
    # Number of points above a table's top edge to scan for caption /
    # section-header text that names benchmarks not repeated in the table
    # cells themselves (common in papers with multi-part result tables).
    _CONTEXT_WINDOW_PT: int = 150
    ...
    def extract(self, pdf_path: Path) -> list[tuple[int, list[list[str]], str]]:
        ...
        *context_text* is the plain text extracted from a window immediately
        above the table bounding box on the same page.  It is passed to
        ``ResultsTableParser.is_results_table`` so that benchmark names in
        captions or section headers can trigger table acceptance even when
        they are absent from the table cells themselves.
        """
        ...
        with pdfplumber.open(str(pdf_path)) as pdf:
            for page_num, page in enumerate(pdf.pages, start=1):
                ...
                for table_obj in page.find_tables():
                    ...
                    # Crop the rectangle above this table to get any caption
                    # or section header text that names the benchmark.
                    # table_obj.bbox = (x0, top, x1, bottom) in pdfplumber's
                    # coordinate system (y increases downward from page top).
                    t_top = table_obj.bbox[1]
                    context_top = max(0.0, t_top - self._CONTEXT_WINDOW_PT)
                    try:
                        context_text: str = (
                            page.crop(
                                (0, context_top, page.width, t_top),
                                relative=False,
                            ).extract_text()
                            or ""
                        )
                    except Exception:  # noqa: BLE001 — bbox clipping errors must not abort
                        context_text = ""
                    tables.append((page_num, cleaned, context_text))
        if total_pages > 0 and blank_pages / total_pages > 0.5:
            print(
                f"  [scanned-pdf] {blank_pages}/{total_pages} pages returned no "
                f"extractable text — PDF appears to be scanned. Table extraction "
                f"is likely incomplete. Re-run with --llm-fallback and "
                f"pytesseract + Tesseract-OCR installed for OCR support.",
                file=sys.stderr,
            )
        return tables
...
class ProseExtractor:
    ...
    * Figure captions: ``"GPT-4 obtains 72.1 on HumanEval (Figure 3)"``
    ...
    def extract(self, pdf_path: Path, arxiv_id: str) -> list[dict[str, Any]]:
        ...
        Reads the full page text produced by pdfplumber.  Pages with no text
        layer are silently skipped — OCR is the responsibility of
        ``LLMFallbackExtractor`` (use ``--llm-fallback``).
        """
        results: list[dict] = []
        ...
        try:
            with pdfplumber.open(str(pdf_path)) as pdf:
                for page in pdf.pages:
                    ...
                    if not text.strip():
                        continue
                    self._scan_page(text, results, seen)
...
class LLMFallbackExtractor:
    """Extract benchmark results from paper text when pdfplumber yields nothing.
    ...
    def extract(self, pdf_path: Path, arxiv_id: str) -> list[dict[str, Any]]:
        ...
        """
        pages: list[str] = []
        _ocr_available: bool | None = None  # None = not yet probed
        try:
            with pdfplumber.open(str(pdf_path)) as pdf:
                for page in pdf.pages:
                    text = page.extract_text() or ""
                    if text.strip():
                        pages.append(text)
                        continue
                    # No text layer on this page — likely a scanned image.
                    # Try OCR via pytesseract when the dependency is available.
                    if _ocr_available is None:
                        try:
                            ...
                            print(
                                "  [llm-fallback] page has no text layer; "
                                "install pytesseract + Tesseract-OCR for "
                                "scanned-PDF support",
                                file=sys.stderr,
                            )
                    if _ocr_available:
                        try:
                            ...
                            print(
                                f"  [llm-fallback] OCR failed on "
                                f"page {page.page_number}: {ocr_exc}",
                                file=sys.stderr,
                            )
        except Exception as exc:
...
def _detect_eval_library(pdf_path: Path) -> EvalLibrary:
    ...
    try:
        with pdfplumber.open(str(pdf_path)) as pdf:
            for page in pdf.pages[:8]:  # abstract + introduction is usually ≤8 pages
